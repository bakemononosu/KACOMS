...
function requestGrading() {
    ...
    processingMessage('採点中です。しばらくお待ちください。');
...
function createSendRequest() {
    ...
    $(`div[id^='t_no_']`).each(function (index, testNo) {
        var no = testNo.id.match(/\d+$/)[0]; 
        $(`div[id='q${no}_tab_pane']`).each(function (index, element) {
            var question = {};
            var aList = [];
            ...
            $(element).find(`div[id^='a_${no}_'`).each(function (index, element) {
                var answer = {};
    ...
    request['QuestionCount'] = qList.length;
...
/**
 * 解答チェックボックス変更イベントハンドラ登録
 */
$(`input[type='checkbox']`).each(function (i, elm) {
    elm.addEventListener('change', updateAnswerStatus);
});
...
function ChangeTest() {
    ...
    var testContentsId = `q${idx}_tab_pane`;

    // 問題を全て非表示
    $(`[id$='tab_pane']`).each(function (i, e) {
        $(e).addClass('d-none');
    });
...
function updateAnswerStatus() {
    var q = $(this).data().parent.match(/q(\d+)_tab_pane/)[1];

    // 解答チェック数を取得
    var answer = $(`#${$(this).data().parent}`)
        .find(`div[id^='a_${q}_'`)
        .find(`input[type='checkbox']:checked`);

    $(`#t_no_${q}`).removeClass('test-no-answered test-no-noanswered');

    // 解答チェック数 ＞ 0の場合、問題セレクタの背景色を変える
    if (answer.length > 0) {
        $(`#t_no_${q}`).addClass('test-no-answered');
    } else {
        $(`#t_no_${q}`).addClass('test-no-noanswered');
    }
}
